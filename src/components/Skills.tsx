...
import { Beaker, FlaskConical, Cog, FileSearch, CircleDashed, Wrench } from 'lucide-react';
...
const Skills: React.FC = () => {
  const skills: SkillType[] = [
    { id: 'organic-synthesis', name: 'Organic Synthesis', level: 95, category: 'synthesis', icon: <Beaker className="h-4 w-4" /> },
    { id: 'lipid-design', name: 'Novel Lipid Design', level: 92, category: 'synthesis', icon: <Beaker className="h-4 w-4" /> },
    { id: 'medicinal-chem', name: 'Medicinal Chemistry', level: 90, category: 'synthesis', icon: <FlaskConical className="h-4 w-4" /> },
    { id: 'peptide-design', name: 'Peptidomimetic Design', level: 87, category: 'synthesis', icon: <FlaskConical className="h-4 w-4" /> },
    { id: 'air-sensitive', name: 'Air-Sensitive Reactions', level: 85, category: 'techniques', icon: <CircleDashed className="h-4 w-4" /> },
    { id: 'high-pressure', name: 'High-Pressure Reactions', level: 82, category: 'techniques', icon: <CircleDashed className="h-4 w-4" /> },
    { id: 'nmr', name: 'NMR Spectroscopy', level: 88, category: 'analytical', icon: <Cog className="h-4 w-4" /> },
    { id: 'hplc', name: 'HPLC Analysis', level: 86, category: 'analytical', icon: <Cog className="h-4 w-4" /> },
    { id: 'scifinder', name: 'SciFinder', level: 89, category: 'software', icon: <FileSearch className="h-4 w-4" /> },
    { id: 'reaxys', name: 'Reaxys', level: 84, category: 'software', icon: <FileSearch className="h-4 w-4" /> },
  ];
    ...
    <div className="section-reveal py-8">
      ...
      <div className="grid grid-cols-1 gap-6">
        {categories.map(category => (
          <Card key={category.id} className="neo-glass border-purple-600/20">
            ...
            <CardContent className="p-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {skills
                  ...
                  .map(skill => (
                    <div key={skill.id} className="space-y-2 p-3 rounded-lg bg-purple-50/50 dark:bg-purple-950/10 border border-purple-100/50 dark:border-purple-800/20">
      ...
      <div className="mt-8 bg-gradient-to-r from-purple-50 to-indigo-50 dark:from-purple-950/10 dark:to-indigo-900/10 p-6 rounded-lg backdrop-blur-sm border border-purple-200 dark:border-purple-900/20">
